...
class IndexModule:
    ...
    def construct_postings_lists(self):
        ...
        files = listdir(config['DEFAULT']['doc_dir_path'])    #返回路径下的文件名字列表
        AVG_L = 0
        for i in files:
            ...
            seg_list = jieba.lcut(title + '。' + body, cut_all=False) #标点符号也会保留
            
            ld, cleaned_dict = self.clean_list(seg_list) #ld：为该文档中最终剩余词的总数 包含重复出现的个数
            ...
            for key, value in cleaned_dict.items():
                ...
                if key in self.postings_lists:
                    self.postings_lists[key][0] = self.postings_lists[key][0] + 1 # df++ 记录词在哪些文档里出现过
